...
import static de.ksquared.jds.Utilities.isNull;
...
import de.ksquared.jds.components.Configurable.Option.OptionType;
...
import de.ksquared.jds.contacts.ContactList.Alignment;
 ...
 * JDigitalSimulator.
 * It was tried to explain a new behaviour and/or feature of the JDigitalSimulator in every single
 * line of coding. Due to this many coding fragments are may not requried to create the plugin
 ...
 *   <code>extend de.ksquared.jds.components.Component</code>. //XXX
 ...
 * But, guess what... We will go into detail of those aswell.
 */
public class ExamplePluginFull extends Component implements Sociable,Configurable,Interactable {
	 ...
	 * You should put serialVersionUID into each of your plugins aswell.
	 */
   ...
   *                                    group.additional, group.others or any other group name.
	 ...
	 * which is necersarry to create an appropriate <code>componentAttributes</code> attribute.
	 */
	public static final ComponentAttributes componentAttributes = new ComponentAttributes("Example-Plugin","group.general","This is an template component for how to create an own plugin.","Kristian Kraljic (kriskra@gmail.com)",1);
	 ...
	 *       likely get called when something changes on the frontend, but it is not guranteed in
	 *       any ways, that it: a) is called every time b) called in fixed time intervalls.
	 ...
	 * unique location in your component, so it is recommened to initialize it right when the
	 * component gets created in the constructor. But for this example we will do it in the
	 * <code>getContacts</code> method directly, to show that also a dynamic ammount of Contacts is
	 ...
	 *       usefull Methods to work with Conacts like the static methods <code>concatenateContacts</code>
	 ...
	 *        later on. Please to not experiment with this method, simly return all of the contacts
	 *        of your component, no matter what or where there are placed.
	 */
	@Override public Contact[] getContacts() {
		if(Utilities.isNull(input)) { /** use the utility methods of the class <code>Utilities</code> */
			/** create a new input contact, you will have to pass your component and a default loction */
			input = new InputContact(this,new Point(10,10));
			input.setLocation(new Point(20,20)); /** you may move the contact arround later on */
		...
		if(outputs==null||outputs.length==0) { /** please be carefull with array to not generate any exceptiosn */
			/** create a new array of output contacts, we will arange them later on automatically */
			outputs = new OutputContact[] { new OutputContact(this), new OutputContact(this) };
			/** pleace all the created output contacts dynamically in an horizontal aligment at the
			 * top of the component. You may use other constructors to modify the pacement of the Contacts */	
			ContactList.setContactLocations(this,outputs,Alignment.HORIZONTAL);
		}
		if(isNull(moreOutputs)) { /** I would even recommend to use static imports for some methods */
	 ...
	 * To enable a great possibility to caculate your component it is absolutely guranteed that
	 ...
	 * Lets begin implementing the getOptions method. Is is recommonended to buffer the options you want
	...
	/** The getConfiguration method has to return the actual configuration for all options everytime it is
	...
	@Override public Map<Option,Object> getConfiguration() {
		...
		return configuration; /** returnt the configuration, the next step is now likely the call of setConfiguration */
	}
	 ...
	 * sattisfied with one or more values the user entered, you may throw a <code>PropertyVetoException</code>,
	...
	@Override public void setConfiguration(Map<Option,Object> configuration) throws PropertyVetoException {
		...
		this.text = configuration.get(options[1]).toString(); /** or you use the apropriate cast-methods */
 ...
 * of your JDigitalSimulator (were you have propably found this file aswell) and start the
...
/* I hope you enjoyed my small tutorial, if you have any further questions or comments please
 * feel free to contact me at kriskra@gmail.com. Thanks and best regards, Kristian Kraljic */
//   COPYRIGHT 2010 (c) - Kristian Kraljic - kSquared Development (k²)   //